...
func find_terrain_rec(node: Node3D) -> void :
	...
	if (parent == null) :
		print("TerrainObstacle::find_terrain_rec() : couldn't find terrain node");
		assert(false);
		return;
...
func set_tile_as_obstacle() -> void :
	...
	if (Vector2(grid_coords) != coords_2d) :
		print("TerrainObstacle: ", name, " is not placed inside a cell!");
		return;
